...
def cesh_bernoulli(means_matrix: np.ndarray, 
    ...
    for halving_step in range(depth):
        ...
        for i in range(num_arms_in_step):
            ...
            mean_0 = sample_sum_bernoulli(sample_num, means_matrix[item_idx_0, feature_idx_i])
            mean_i = sample_sum_bernoulli(sample_num, means_matrix[item_idx_i, feature_idx_i])
...
def cr_bernoulli(means_matrix: np.ndarray, delta: float) -> Tuple[List[int], int]:
    ...
    while not terminate:
        ...
        while (halving_step * 2**(halving_step + 1) <= budget_iteration and halving_step < max_halving_steps):
            ...
            # Estimate means using sum of Bernoulli samples
            mean_0 = sample_sum_bernoulli(samples_per_arm, means_matrix[item_idx_0, feature_idx_cand])
            mean_cand = sample_sum_bernoulli(samples_per_arm, means_matrix[item_idx_cand, feature_idx_cand])
...
def cluster_bernoulli(means_matrix: np.ndarray, delta: float) -> Tuple[Optional[np.ndarray], int]:
    ...
    # Check using sum of Bernoulli samples
