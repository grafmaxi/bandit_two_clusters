...
def cesh_gaussian(means_matrix: np.ndarray,
    ...
    for halving_step in range(depth):
        ...
        indices[0] = [indices[0][i]
                      for i in index_sort[: int(2**(depth - halving_step - 1))]]
...
def cr_gaussian(means_matrix: np.ndarray,
    ...
    while not terminate:
        ...
        while (
            ...
            candidate_row_result = cesh_gaussian(means_matrix, range(
                num_items), halving_step, 2**(iteration + 1))
...
def cbc_gaussian(means_matrix: np.ndarray,
    ...
    while clusters[0] == 0:
        ...
        while (
            ...
            if (abs(np.random.normal(int(2**iteration /
                                         num_items) *
                                     (means_matrix[0, candidate_column_index] -
                                      means_matrix[candidate_row_index, candidate_column_index]), np.sqrt(2 *
                                                                                                          int(2**iteration /
                                                                                                              num_items)))) > 3 *
                np.sqrt(4 *
                        int(2**iteration /
                ...
                for ind in range(1, num_items):
                    if (abs(np.random.normal(int(2**iteration /
                                             ...
                                             (means_matrix[candidate_row_index, candidate_column_index] -
                                              means_matrix[ind, candidate_column_index]), np.sqrt(2 *
                                                                                                  int(2**iteration /
                                                                                                      num_items)))) > np.sqrt(4 *
                                                                                                                              int(2**iteration /
                                                                                                                                  num_items) *
...
def cluster_gaussian(means_matrix: np.ndarray,
    ...
    if (means_matrix[0, :] == means_matrix[candidate_indices[0], :]).all():
        ...
        clusters, cbc_cost = cbc_gaussian(
            means_matrix, candidate_indices[0], delta / 2)
