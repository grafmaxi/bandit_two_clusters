...
The algorithms use Binomial draws for sampling.
"""
import numpy as np
...
def cesh_bernoulli(means_matrix: np.ndarray,
    ...
    for halving_step in range(depth):
        ...
        for i in range(num_arms_in_step):
            ...
            mean_0 = np.random.binomial(
                sample_num, means_matrix[item_idx_0, feature_idx_i])
            mean_i = np.random.binomial(
        ...
        while (halving_step * 2**(halving_step + 1) <=
            ...
            if samples_per_arm > 0:
                ...
                if diff > threshold:
                    ...
                    terminate = True
                    # Ensure candidate_indices has the right format before
                    # returning
...
def cbc_bernoulli(means_matrix: np.ndarray,
    ...
    while not found_feature:
        ...
        while (halving_step * 2**(halving_step + 1) <=
            ...
            samples_per_comp = max(1, int(2**iteration / num_items))

            # Compare item 0 and candidate_row_index using the candidate
            # feature
            mean_0 = np.random.binomial(
            ...
            if abs(mean_0 - mean_cand) > threshold_main:
                ...
                # Assign candidate to cluster 1
                clusters[candidate_row_index] = 1

                # Now, compare all other items to the candidate_row_index using
                # this feature
                threshold_others = np.sqrt(
                    samples_per_comp *
                    np.log(log_thresh_arg))  # Threshold for other items

                for ind in range(num_items):
                    if ind == 0 or ind == candidate_row_index:
                        continue  # Skip items already assigned

                    # Compare item 'ind' with candidate_row_index
                    mean_ind = np.random.binomial(
                        samples_per_comp, means_matrix[ind, candidate_column_index])
                    mean_cand_again = np.random.binomial(
                        samples_per_comp, means_matrix[candidate_row_index, candidate_column_index])
                    cost += 2 * samples_per_comp

                    # If item 'ind' is different from candidate_row_index,
                    # assign to cluster 0
                    if abs(mean_cand_again - mean_ind) > threshold_others:
                        clusters[ind] = 0
                    # Otherwise, assign to cluster 1 (same as
                    # candidate_row_index)
                    else:
                        clusters[ind] = 1
        ...
        # Safety break
        if iteration > 100 + \
                int(np.ceil(np.log(num_items) / np.log(2))):  # Adjust limit
...
def cluster_bernoulli(means_matrix: np.ndarray,
    ...
    Returns:
        ...
        - cost: Total number of samples that were used.
    """
    sample_cost = 0
