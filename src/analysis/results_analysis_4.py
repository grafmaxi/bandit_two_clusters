...
mean_cluster_budgets = np.array([np.mean(budgets) if budgets else 0 for budgets in cluster_budgets])
...
lower_quantiles_cluster_budgets = np.array([np.quantile(budgets, qlow) if budgets else 0 for budgets in cluster_budgets])
upper_quantiles_cluster_budgets = np.array([np.quantile(budgets, qhigh) if budgets else 0 for budgets in cluster_budgets])
...
ax1.plot(feature_grid, mean_adaptive, label='Error rate for Adaptive Clustering', color='blue', linewidth=2)
ax1.plot(feature_grid, mean_cluster_errors, label='Error rate for BanditClustering', color='red', linewidth=2)
ax1.set_ylabel('Error Rate', color='black')
ax1.set_ylim(bottom=0, top=0.6)
ax1.tick_params(axis='y', labelcolor='black')
ax1.set_xlabel('Number of features')

# Plot budgets on right y-axis
ax2.plot(feature_grid, mean_cluster_budgets, label='Average budget for BanditClustering', color='red', linewidth=2, linestyle=':')
ax2.plot(feature_grid, np.full(feature_size, 400000), label='Budget used for Adaptive Clustering', color='blue', linewidth=2, linestyle=':')
ax2.set_ylabel('Budget', color='black')
...
ax1.legend(lines1, labels1, bbox_to_anchor=(0.05, 0.65), loc='center left', title='Error Rates')
ax2.legend(lines2, labels2, bbox_to_anchor=(0.32, 0.2), loc='center left', title='Budgets')
plt.tight_layout()
