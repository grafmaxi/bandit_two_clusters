...
budgets = [[[] for _ in range(len(delta_array))] for _ in range(len(num_items_array))]
...
widths = [1, 1, 1, 2]
plt.figure(dpi=300)  
plt.plot(np.array(range(num_items_array[-1])), np.array(range(num_items_array[-1]))**2,
...
for i in range(len(delta_array)):
    ...
    plt.plot(num_items_array, temp_mean_budgets, label=fr'$\delta$={delta_array[i]}', 
...
plt.fill_between(num_items_array, lower_quant, upper_quant, 
                 color='red', alpha=0.2, 
