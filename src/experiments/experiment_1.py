...
def simulation_iteration_1(seed: int) -> Tuple[np.ndarray,
                                               np.ndarray,
                                               np.ndarray,
                                               np.ndarray,
                                               np.ndarray,
    ...
    for s_idx in range(sparsity_grid_size):
        # Calculate sparsity
        sparsity = int((config_1["num_features"] - 1)
                       * s_idx / (sparsity_grid_size - 1) + 1)
        ...
        for budget_idx in range(budget_steps):
            ...
            km_errors[s_idx, budget_idx] = 1 - \
                int(clusters_equal(true_clusters, km_clusters))
